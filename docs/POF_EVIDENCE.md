...
| Giấy phép OSI-approved | 10 | `LICENSE`, `LICENSES/Apache-2.0.txt`, `LICENSES/MIT.txt`, `NOTICE`, `THIRD_PARTY_NOTICES.md`, SPDX/REUSE | Mã FocusMate có phạm vi rõ. License của hai model MediaPipe đúng byte vẫn `NOASSERTION`; đây là blocker release binary cho tới khi có nguồn artifact-specific. |
...
| Thư viện và bundling | 10 | lockfile, verification metadata, SBOM lịch sử + SBOM candidate, provenance, notices, manifest model | SBOM candidate có 86 package và đủ 8 component firmware; audit POM đúng version đã giảm `NOASSERTION` từ 69 xuống đúng 2 model. `esp_new_jpeg` và JSON-java giữ LicenseRef chính xác. Loader upstream giữ nguyên byte; Face task compact có manifest chuyển đổi. |
