...
- `GET /api/wifi/scan` tạo FreeRTOS task scan nền và trả trạng thái `running`; task dùng chế độ background coexistence cùng dwell mặc định mà ESP-IDF yêu cầu khi BLE đang bật, quét từng kênh hợp lệ rồi trả radio về kênh SoftAP 40 ms giữa các lượt. `GET /api/wifi/scan/status` trả `running`, `ready` kèm tối đa 24 mạng hoặc `error`. Scan không còn giữ HTTP request mở và dashboard tự retry polling nếu có channel hop; người dùng cũng có thể bỏ qua scan và nhập SSID thủ công vào `POST /api/wifi/connect`. Các route `POST /api/wifi/connect`, `POST /api/wifi/reset`, `POST /api/wifi/ap-password` có validate length và không echo password.
