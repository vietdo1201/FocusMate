...
data class PendingYawnSyncEvent(
    ...
    fun toJson() = JSONObject().apply {
        ...
        put("observed_esp_uptime_ms", observedEspUptimeMs)
    }

    companion object {
        fun fromJson(json: JSONObject) = PendingYawnSyncEvent(
            eventId = json.optLong("event_id", -1L),
            frameSequence = json.optLong("frame_sequence", -1L),
            observedEspUptimeMs = json.optLong("observed_esp_uptime_ms", -1L),
        ).takeIf {
            it.eventId in 0L..UINT32_MAX && it.frameSequence in 0L..UINT32_MAX &&
                it.observedEspUptimeMs >= 0L
        }
...
private fun newYawnSyncSessionId(): String = UUID.randomUUID().toString().replace("-", "")

private fun newYawnSyncClientId(): Long =
    (UUID.randomUUID().mostSignificantBits and 0xFFFF_FFFFL).coerceAtLeast(1L)
...
data class ActiveStudySession(
    ...
    val yawnSyncSessionId: String = newYawnSyncSessionId(),
    val yawnSyncClientId: Long = newYawnSyncClientId(),
    val yawnSyncRevision: Long = 0L,
    val yawnSyncWindowCount: Int = 0,
    val yawnSyncObservedAtMs: Long? = null,
    val nextYawnSyncEventId: Long = 0L,
    val pendingYawnSyncEvents: List<PendingYawnSyncEvent> = emptyList(),
    val breakTargetMinutes: Int = FocusMateRules.DEFAULT_BREAK_MINUTES,
    val breakCount: Int = 0,
    ...
    fun toJson() = JSONObject().apply {
        ...
        put("yawn_sync_session_id", yawnSyncSessionId)
        put("yawn_sync_client_id", yawnSyncClientId)
        put("yawn_sync_revision", yawnSyncRevision)
        put("yawn_sync_window_count", yawnSyncWindowCount)
        put("yawn_sync_observed_at_ms", yawnSyncObservedAtMs ?: JSONObject.NULL)
        put("next_yawn_sync_event_id", nextYawnSyncEventId)
        put("pending_yawn_sync_events", JSONArray().apply {
            pendingYawnSyncEvents.forEach { put(it.toJson()) }
        })
        put("break_target_minutes", breakTargetMinutes)
        put("break_count", breakCount)
    ...
    companion object {
        fun fromJson(json: JSONObject) = ActiveStudySession(
            ...
            yawnSyncSessionId = json.optString("yawn_sync_session_id")
                .takeIf { it.matches(Regex("[0-9a-f]{32}")) } ?: newYawnSyncSessionId(),
            yawnSyncClientId = json.optLong("yawn_sync_client_id", 0L)
                .takeIf { it in 1L..4_294_967_295L } ?: newYawnSyncClientId(),
            yawnSyncRevision = json.optLong("yawn_sync_revision", 0L).coerceIn(0L, 4_294_967_295L),
            yawnSyncWindowCount = json.optInt("yawn_sync_window_count", 0).coerceIn(0, 64),
            yawnSyncObservedAtMs = json.optionalLong("yawn_sync_observed_at_ms"),
            nextYawnSyncEventId = json.optLong("next_yawn_sync_event_id", 0L)
                .coerceIn(0L, 4_294_967_295L),
            pendingYawnSyncEvents = json.optJSONArray("pending_yawn_sync_events")?.let { array ->
                buildList {
                    for (index in 0 until minOf(array.length(), 16)) {
                        array.optJSONObject(index)?.let(PendingYawnSyncEvent::fromJson)?.let(::add)
                    }
                }
            }.orEmpty(),
            breakTargetMinutes = json.optInt("break_target_minutes", FocusMateRules.DEFAULT_BREAK_MINUTES)
