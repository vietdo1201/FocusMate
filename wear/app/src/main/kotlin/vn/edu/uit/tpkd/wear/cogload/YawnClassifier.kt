...
class YawnClassifier(seed: YawnSeed = YawnSeed()) {
    ...
    fun synchronizeRemote(
        ...
        return snapshot(
            ...
            // A canonical increase came from another source (or from our own
            // previously ACKed outbox event). It must update persistence/UI but
            // must never be re-enqueued as a new local detection, otherwise the
            // Watch and ESP echo the same increase forever.
