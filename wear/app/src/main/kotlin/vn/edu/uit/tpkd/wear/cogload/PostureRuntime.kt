...
class FaceObservationIngestor(
    ...
    fun onNotification(
        ...
        onAuxiliaryPayload: (ByteArray) -> Boolean = { false },
    ): Boolean {
        // Binary extensions can fit in a single negotiated-MTU notification.
        // Give them to the extension decoder before the observation reassembler,
        // whose unframed fast path is intentionally restricted to JSON (`{`).
        // If the extension is fragmented at MTU 23, parsing fails here and the
        // normal reassembly path below still delivers the complete payload.
        if (onAuxiliaryPayload(notification)) return true
        ...
        if (onAuxiliaryPayload(outcome.payload)) return true
